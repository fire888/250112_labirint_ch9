...
export const createWall_01_door = (root: Root, d: number, h: number, doorData: DoorData) => {
    const v: number[] = []
    const uv: number[] = []
    const c: number[] = []
    ...
    { // door 
        const door = createDoor_00(root, doorData.width, doorData.depth, doorData.height)
